...
const modules = import.meta.glob('../img/about/*.{jpg,jpeg,png,JPG,JPEG,PNG}', { eager: true });
...
// Progressive image — skeleton cream → fade in เมื่อโหลดเสร็จ
    ...
    >
      {visible && (
        <>
          <img
            src={src}
            alt={`Wedding makeup ${globalIndex + 1}`}
            loading="lazy"
            decoding="async"
  ...
  return (
    <>
      <style>{`
        ...
        .loaded .about-header { opacity: 1; transform: translate3d(0,0,0); }
        .about-eyebrow {
        ...
        /* Pinterest Masonry */
        .masonry { display: flex; gap: 12px; align-items: flex-start; position: relative; z-index: 1; }
        ...
        .masonry-tile:hover img { transform: translate3d(0,0,0) scale(1.04); }
        .masonry-tile::after {
        ...
        .progress-label { font-size: 0.55rem; font-weight: 200; letter-spacing: 0.25em; text-transform: uppercase; color: var(--muted); }
        .loading-dots span {
        ...
        .lightbox-close {
          ...
          letter-spacing: 0.3em; text-transform: uppercase; color: rgba(245,240,232,0.45); transition: color 0.2s ease;
        }
      ...
      <section className="about-section" ref={sectionRef}>
        ...
        <div className="about-header">
          ...
          <h1 className="about-title">Wedding <em>Makeup</em></h1>
          ...
          {columns.map((col, ci) => (
            <div className="masonry-col" key={ci}>
              {col.map(({ path, globalIndex }) => (
                <MasonryTile key={path} path={path} globalIndex={globalIndex} onClick={setLightbox} />
              ))}
            </div>
          ))}
        </div>
        ...
        <div className="gallery-footer">
          ...
          <span className="progress-label">{visiblePaths.length} / {imagePaths.length}</span>
          {hasMore ? (
            <>
